...
\kyle{This paper is more than generating training data? I think here its a system for extracting 
materials info by applying semi-supervised methods}\roselyne{but here I do want to put the emphasis on the capturing of focused expert knowledge via active learning. So it's generating training data for NER, we do the NER part as well, but we captured our own training data.}

To address these challenges, we propose polyNER, a system for generating training data for scientific NER using semi-supervised learning and focused expert input. 
