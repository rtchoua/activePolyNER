...
Semi-supervised learning attempts to automatically label data by using prior knowledge and a set of labeled data. 
...
Like other prior works, polyNER explores the combination of semi-supervision and active learning~\cite{nguyen2004active,basu2004active}.
In its first stage, our approach is similar to bootstrapping but applied to named entities rather than entity pairs. 
%\logan{Confusing. If PolyNER is similar to boostrapping, why are are you describing it in the active learning section? I'm generally not sure why this paragraph is in "Related Work"}\roselyne{This was its own section, related polyner to all previous sections including bootstrapping and not just active learning, but felt a bit weird to Kyle, so we moved it to motivation, perhaps it fits before current changes to architecture to motivate what we are doing now. Will just try to do this link between our work and related work in each section and have a polyNER paragraph in architecture.}
...
We subsequently use active learning to address noise in the output and continue obtaining more accurate labels.
