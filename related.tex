...
\ian{The reference to ``cluster and label algorithms'' is obscure to me.}
...
We generate approximate labels by extracting targets similar to a few seed entities, 
with similarity determined by using word representations and vector similarity measures.
...
which can be used\ian{the term ``can be used'' confuses me. Is the training that is defined here part of your system?}
 to train a word vector classifier and propose final\ian{what does ``final'' mean?} scientific named entities.
