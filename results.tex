...
We first report on a study in which we evaluate the generation of candidate entities using the vector similarity measures and representative (commonly used) entities. 
...
Recall from Section~\ref{sec:we} that the first step of the polyNER pipeline involves the use of the
...
we explore how the choice of seed entities and the Word2Vec parameters (vector) \texttt{size} and \texttt{window_size} impact the number of target entities retrieved. %  using similarity measures.
...
We train the initial classifier on the 200 entities with the highest similarity scores in the previously discussed list of \num{10000} candidates.
