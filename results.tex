...
% and the Word2Vec parameters (vector) \texttt{size} and \texttt{window_size} 
impact the number of target entities retrieved. %  using distance measures.
...
%\ian{I am not sure what Figure~\ref{fig:knn_init} is showing. Is this the results evaluated on the 20\% of hold out data from the bootstrap set? On the 467 polymers in the test set?}
...
%\label{fig:prc_init}
\end{minipage}
\caption{ROC (left) and PR (right) curves for KNN model for initial round of labels. The PR curve shows that precision is low regardless of recall, indicating that we need more data.%\ian{expand caption.}\label{fig:knn_init}
}
\end{figure*}
...
%\roselyne{There are 5 rounds of labeling but 4 rounds of active learning, it's confusing, with the table this is less confusing, I used to say, round, but changed to round, will use last. Agreed about the names, but hard to compress these names. I've tried and changed them but they haven't changed in the figure yet.}
%}
...
%\ian{you mean after one round?}\roselyne{compared to the graph above, initial labeling}
...
which achieve 42.2\% precision and 68.3\% recall on the same test set.%\ian{rewording ok here?}\roselyne{yes}
...
We perform preliminary labeling tests to estimate time. %and insure agreement between experts.
One expert labels candidates from a subset of documents in our corpus achieving an average of 60 minutes per 200 labels (two labeling sessions of $\sim$30 minutes for 100 labels).%\roselyne{This is the number I remember but I have not written it anywhere}
...
%\ian{Here will be the labeling discussion that Roselyne says she will add.}
