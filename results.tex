...
\roselyne{Since I removed parameter tuning, add choices of parameters here?}\ian{I'd suggest putting it in in Section~\ref{sec:wordembeddings}.}
...
Once those data are labeled by our experts, we train our initial KNN classifier.
\ian{I am not sure what Figure~\ref{fig:knn_init} is showing. Is this the results evaluated on the 20\% of hold out data
from the bootstrap set? On the 467 polymers in the test set?}
...
We selected seed entities based on their frequency in our corpus and the yield of target entities during sampling.\ian{I thought that you said above that you picked the most frequent words?}
...
Next, we train a KNN classifier using vectors for these candidates labeled through active learning from the NLP-filtered candidates (identified as best-performing in previous experiments).
...
and intermediate\ian{I don't know how to interpret this word.} precision, with only about five hours of expert labeling.
