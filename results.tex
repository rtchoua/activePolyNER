...
%\ian{I think that having said above how we define performance, 
%we can say ``We first investigate how the choice of seed entities affects performance.''}
...
When using \textit{polystyrene} (the most frequently used name) as a seed entity, the pool of candidates contained 33.55\% of the 467 gold standard polymers.
...
\ian{Can this be reworded to ``We train the initial classifier on the 200 entities 
the highest similarity scores
in the previously discussed list of
\num{10000} candidates.'' [There is no need to explain how you found those entities.]}
...
%\ian{You previously said 60 minutes. Be consistent.}
...
We show the precision recall curve for the FastText vector classifier and the CDE+ performance on Figure~\ref{fig:UBS_prcs_fasttext}.
...
These candidates are more likely to lack context. % and for which vectors have been generated using character-level information. 
%\ian{I don't see why it is a result that the expert often presented with nearly identical candidates.}\roselyne{I've tried to re-add the character information to add sense but without the long sentence}
and their corresponding vectors have been generated using mostly character-level information.
