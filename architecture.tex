...
the normal process and then perhaps introduce some baselines for comparison in the next section.}
...
In the first strategy we randomly select 200 out of the pool of unlabeled NLP-filtered candidates \kyle{not known what an unlabeled NLP-filtered candidate is}. Note that the imbalance between polymers and other \textit{tokens} (words or space separated strings) that do not exist in in this set is still significant (less than 5\% in our test set).
...
\caption{\label{fig:current} The active learning process. \kyle{more detail needed here. I think the pic is missing some arrows too. We could make it horizontal to ensure that it uses less space}}
...
\kyle{Confusing to have AL exp setup here}
...
\kyle{Here we might want to think about positioning this as 2-phase review where we combine humans with different expertise levels}
...
\kyle{We should talk about how to structure this section.}
