...
\ian{Are the three strategies all an integral part of polyNER, or are they alternatives that we compare? Unclear. I reworded to say they are alternatives, is that right?}
...
pool, the ``NLP-filtered candidates."}
...
\caption{\label{fig:architecture} PolyNER system showing NLP-filtering in (1), initial sampling and labeling in (2), candidate sampling strategies in (3), and active learning loop between expert labeling (4) and classification of scientific named entities (5). 
...
words\ian{Maybe the wrong term, as the first example is ``polyethylene;'', which is not a ``word'' but a string?} in scientific publications that are unlikely to be polymer referents. 
...
While the previous steps\ian{Should this be singular?}
...
\ian{You refer to similarity metrics in one sentence and distance metric in the next. Be consistent!}
...
in which 200 candidates are selected at random from the NLP-filtered candidates.''}
...
\ian{Why can't you have a probability of 1, for say ``polystyrene''?}
...
As previously mentioned, we save\ian{that word save again.} the \textit{best-performing} classifier on labeled candidates (prior to updating the word embedding model  with test documents in order to clearly separate the training process from the test set, see step 1 in Figure~\ref{fig:current}).
