...
\kyle{Would be good to have some more padding on the NL filtering step. Also not sure ``cut numbers, vocab'' is meaningful? Sampling strategies has the m from random on the second box. Again, maybe some padding there would be clearer.}
}
\end{figure*}
...
\kyle{Also is the figure wrong. Aren't the sampling strategies part of the active learning loop?}
